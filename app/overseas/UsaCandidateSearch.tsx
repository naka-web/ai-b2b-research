...
import { MAX_CANDIDATES, MAX_KEYWORDS, USA_KEYWORDS } from '@/services/overseas/candidates/conditions';
...
export default function UsaCandidateSearch({ disabled, onSelect }: { disabled: boolean; onSelect: (candidate: UsaCandidate) => void }) {
  const [keywords, setKeywords] = useState<string[]>(['matcha importer', 'matcha distributor', 'Japanese green tea importer']);
  ...
  async function search() {
    ...
    setError('');
    const key = JSON.stringify([...keywords].sort());
    ...
    try {
      const response = await fetch('/api/overseas/candidates', { method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ country: 'US', keywords, limit: MAX_CANDIDATES }), cache: 'no-store' });
  ...
  return <section aria-label="USA候補検索" className="space-y-4 rounded-xl border border-sky-200 bg-white p-5">
    <h2 className="text-xl font-semibold">USA候補検索</h2>
    <p className="text-sm text-slate-600">Google PlacesからUSA所在地のある企業・事業所候補を探します。検索一致だけでは日本産・B2B・A/B/Cを確定しません。公式サイト候補も未確認です。</p>
    <fieldset disabled={busy || disabled} className="flex flex-wrap gap-x-5 gap-y-2">
      ...
      {USA_KEYWORDS.map(keyword => <label key={keyword} className="flex items-center gap-2 text-sm"><input type="checkbox" checked={keywords.includes(keyword)} disabled={!keywords.includes(keyword) && keywords.length >= MAX_KEYWORDS}
        onChange={e => setKeywords(old => e.target.checked ? [...old, keyword] : old.filter(k => k !== keyword))} />{keyword}</label>)}
    </fieldset>
    <button type="button" onClick={search} disabled={busy || disabled || !keywords.length} className="rounded bg-sky-800 px-5 py-2 text-white disabled:opacity-50">{busy ? '候補検索中…' : 'USA候補を検索'}</button>
    ...
    {result && <>
      ...
      <div className="overflow-x-auto rounded border border-slate-200"><table className="w-full min-w-[1100px] text-left">
        <caption className="p-3 text-left">USA候補一覧 — 商品・法人・B2Bの最終確認は公式サイト調査で行います</caption>
