...
## Germany / France / Netherlands追加（2026-09-09）

候補検索の対象国にUSA・Germany・France・Netherlandsを表示。`countryProfiles.ts` の `candidateSearch` に国コード、検索地域名、検索語と初期選択を集約した。
USAの検索語・初期選択は維持。追加3か国は英語と現地語の6検索語から最大3個を選択する。

- Google Placesの`regionCode`と検索文の国名を選択国に合わせる。返された住所の国コードが選択国と一致しない候補は除外。
- 3検索語・各10件・合計30候補、重複除外、タイムアウト、30秒の検索間隔、保存制約、provider契約は維持。
- 国切替で旧候補一覧をクリアし、同条件判定にも国コードを含める。候補選択時に候補の国・企業名・URLを調査フォームへ渡す。
- Netherlandsを既存の調査APIの受入国に追加し、公式住所の郵便番号形式（例: 1234 AB Amsterdam）に対応。国内・FDA・既存3か国の住所判定を変更しない。
- 日本産不明でも候補は表示・選択できる。候補に原産地やA/B/Cを付けず、検索一致をB2B等の根拠にしない。既存A/B/Cルール自体は今回変更していないため、公式調査後のAには従来の日本産条件が残る。
- 互換性と差分を小さくするため、既存ファイル/型名の`UsaCandidateSearch`・`UsaCandidate`は維持し、扱う国を拡張している。

追加テストは4か国の検索地域・国フィルタ・重複・受け渡し、日本産不明でもB候補を維持すること、国ごとの検索語制約、国を跨ぐ非統合、Netherlands公式住所を確認する。

### 継続検証（2026-09-09）

各国の初期選択3検索語で実APIを実行（各3要求）。USA 8候補・重複除外7件、Germany 17候補・重複除外7件、France 15候補・重複除外4件、Netherlands 13候補・重複除外17件。表示住所の対象国対応と国切替時の旧一覧クリアを確認。Netherlandsの同条件再クリックは追加0要求。

各国1候補で「この企業を調査」から国・企業名・URLがフォームに一致して渡ることを確認し、「調査開始」まで実行。4か国とも既存調査から画面内の結果表示へ到達（外部HTTPは順に7・8・2・7回）。USAは完了、他3か国は既存判定で確認待ち。候補由来の結果は永続保存されない表示を確認。

実検索でCalendly予約URLが公式サイト候補に入る問題を確認し、既存URL除外リストに`calendly.com`を追加。企業候補自体は残し、公式URL未確認として扱う。予約URL・サブドメインの除外と無関係なドメインを除外しない回帰確認を既存テストに追加。

`npm run build`成功、`node --test tests/*.test.mjs`は55件成功、`git diff --check`成功。実候補の企業名・URLはファイル保存していない。
